...
| `Package::MODULE_ADDED`                | The status above applied, or a module implements `ExecutableModule`                                                                                                                                                      |
| `Package::MODULE_NOT_ADDED`            | The status above did not apply for a modules that is non-executable. That might happen in two scenarios: a module only implemented base `Module` interface, or did not return any service.                               |
